...
import openpi.policies.pickgreen_policy as pickgreen_policy
...
@dataclasses.dataclass(frozen=True)
class LeRobotPickGreenIntoBowlDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        # replace the transforms below with your own.
        data_transforms = _transforms.Group(
            inputs=[pickgreen_policy.PGInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[pickgreen_policy.PGOutputs()],
        )
...
_CONFIGS = [
    ...
    # the comments below.
    TrainConfig(
        name="pi0_pickgreenintobowl_low_mem_finetune",
    ...
    TrainConfig(
        name="pi0_fast_pickgreenintobowl_low_mem_finetune",
    ...
    TrainConfig(
        ...
        data=LeRobotLiberoDataConfig(
            repo_id="your_hf_username/libero",  # change this to the location of the converted dataset
            base_config=DataConfig(prompt_from_task=True),
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader("gs://openpi-assets/checkpoints/pi0_fast_base/params"),
        num_train_steps=30_000,
